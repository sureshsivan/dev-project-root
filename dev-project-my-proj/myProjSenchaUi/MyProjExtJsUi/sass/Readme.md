...
folder are mapped to the application's root namespace, 'MyProjExtJsUi'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "MyProjExtJsUi"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'MyProjExtJsUi' in the same way
as `"MyProjExtJsUi/sass/src"`.
